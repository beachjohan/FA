...
\end{proof}
The equality holds (proof) iff \todo{$y_j$?} $\arg x_iy_i$  and $|x_j|^p|y_j|^p$ are independent of $j$. For any $x\in \ell^p$ we can choose $y\in \ell^q$ so we get equality (proof?). Hence we get 
\todo{$xu$ ?} $$ \|x\|_p = \max\limits_{\substack{\|u\|_q = 1}}\|(xu)\|_1$$
