...
\tableofcontents
\newpage
\section{Basic results in Topology}
...
(Maximal element:) Nor according to Zorns lemma, $A$ has an maximal element, $M$. This is linearly independent. Let $<M>$ be the span of $M$. Assume there is an $x\in X\setminus<M>$ let $(x_1,\ldots, x_n\in \subseteq M$ be a finite collection of vectors such that 
$$ a_1x_1 + \ldots + a_nx_n - a_{n+1}x = 0,$$such that $a_{i} = 0$ then $x\cup M$ would be linearly independent contradicting the maximality of $M$. On the other hand if $a_{n+1} \neq 0$  $x = \frac{1}{a_{n+1}} \sum a_ix_i$ contradicting $x\not\in <M>$ Hence $x\not \in M$ does not exist and $X = <M>$
\end{proof}
